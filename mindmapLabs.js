(function(){
  ...
  function render(host,collapsed){
    ...
    const btn=n=>{const isParent=(map[n.id]||[]).length>0;const active=col.has(n.id);const s=style(n.type,active);return `<button type="button" data-mm-node="${n.id}" style="position:absolute;left:${n.x}px;top:${n.y}px;width:${n.w}px;height:${n.h}px;background:${s.bg};color:${s.fg};border:2px solid ${s.bd};border-radius:22px;padding:0 26px;text-align:left;display:flex;align-items:center;cursor:${isParent?"pointer":"default"};font-family:Poppins, ui-sans-serif;letter-spacing:0.2px;font-size:22px;font-weight:800;line-height:1.15;box-shadow:0 18px 60px rgba(15,23,42,0.18);transition:transform 160ms ease, box-shadow 160ms ease, filter 160ms ease;outline:none">${n.label}</button>`;};
    const ctrlBtn=(label,attr)=>`<button type="button" ${attr} style="height:68px;min-width:68px;padding:0 26px;border-radius:22px;border:2px solid rgba(15,23,42,0.10);background:linear-gradient(180deg, #ffffff, #eef4ff);color:#0f172a;font-size:18px;font-weight:900;cursor:pointer;font-family:Poppins, ui-sans-serif;box-shadow:0 18px 60px rgba(15,23,42,0.20);transition:transform 160ms ease, background 160ms ease, box-shadow 160ms ease;outline:none">${label}</button>`;
